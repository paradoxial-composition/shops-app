...
exports.nearby_user_shop = function (req, res) {
  DislikedShops.find({ userId: req.params.id }, function (err, shop) { // we look for all the shopIds that this user has liked/disliked
    if (err) {
      res.send(err)
    } else {
      let i
      ...
      PrefferedShops.find({ userId: req.params.id }, function (err, shop) { // we look for all the shopIds that this user has liked
        if (err) {
          res.send(err)
        } else { // our array shop is a json object array wich contains all the shopId s our user has liked and disliked
          for (i = 0; i < shop.length; i++) { // we extract the shopid s from the json object and gather them into an array
            userShopList.push(shop[i].shopId)
          }
          let radius = req.params.rd
                ...
                Shops.aggregate([ // this will fetch us every shop whom wich id is not in the userShopList array and are nearby the user's position
                  {
                    $geoNear: {
                      ...
                      distanceField: 'distance',
                      maxDistance: parseInt(radius) || 10000, // if radius is null then default distance is 10Km
                      query: { _id: { $nin: userShopList } } // agregate will skip any _id that is in the userShopList array
                    }
                  }
                ], function (err, shop) {
                  if (err) {
